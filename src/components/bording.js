...
export default function LandSelling(){
    ...
    const setUpload= () => {
      ...
      useEffect(() => {
          axios.get("http://localhost:8075/advertising/searchBording/"+nic)
    ...
    <BeforeNav/>
     <div>
      ...
      <MDBTableBody>
             {lands.map((bording,key) => (
        <tr>
          <td>{bording.title}</td>
          <td>{bording.tel}</td>
          <td>{bording.discription}</td>
          <td>{bording.price}</td>
          <td>{bording.disctrict}</td>
          <td>{bording.address}</td>
          <td>{bording.status} <span onClick={() => updateStatus(bording.id)}><i class="fas fa-pen"></i></span></td>
          <td>
            <div className="row">
               <div className="col-6"> 
                    <MDBBtn tag='a' onClick={() => remove(bording.id)} color='none' className='m-1 text-danger'>
                            <i className="fas fa-trash"></i>
                    </MDBBtn>
               </div>
                <div className="col-6">
                    <Link  onClick={() => update(bording.id,bording.owner,bording.title, bording.tel, bording.discription, bording.price, bording.disctrict, bording.address, bording.image)} >
