...
export default function LandSelling(){
  ...
  const queryString = window.location.search;
    ...
    const [BussinessPlaces,setBussinessPlaces] = useState([]);
      useEffect(() => {
          axios.get("http://localhost:8075/advertising/searchBussinesss/"+nic)
          .then(res => setBussinessPlaces(res.data))
  ...
  function  updateStatus(id) {
    ...
    <BeforeNav/>
     <div>
      ...
      <MDBTableBody>
             {BussinessPlaces.map((BussinessPlaces,key) => (
        <tr>
          <td>{BussinessPlaces.title}</td>
          <td>{BussinessPlaces.tel}</td>
          <td>{BussinessPlaces.discription}</td>
          <td>{BussinessPlaces.price}</td>
          <td>{BussinessPlaces.disctrict}</td>
          <td>{BussinessPlaces.address}</td>
           <td>{BussinessPlaces.status} <span onClick={() => updateStatus(BussinessPlaces.id)}><i class="fas fa-pen"></i></span></td>
          <td>
            <div className="row">
               <div className="col-6"> 
                    <MDBBtn tag='a' onClick={() => remove(BussinessPlaces.id)} color='none' className='m-1 text-danger'>
                            <i className="fas fa-trash"></i>
                    </MDBBtn>
               </div>
                <div className="col-6">
                    <Link onClick={() => update(BussinessPlaces.id,BussinessPlaces.owner,BussinessPlaces.title, BussinessPlaces.tel, BussinessPlaces.discription, BussinessPlaces.price, BussinessPlaces.disctrict, BussinessPlaces.address, BussinessPlaces.image)} >
