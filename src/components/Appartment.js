...
export default function LandSelling(){
  ...
  const queryString = window.location.search;
    ...
    const [apartments,seApartments] = useState([]);
      useEffect(() => {
          axios.get("http://localhost:8075/advertising/searchAppartment/"+nic)
          .then(res => seApartments(res.data))
     ...
     <div>
      ...
      <MDBTableBody>
        ...
        <tr>
          ...
          <td>{apartment.tel}</td>
          <td>{apartment.discription}</td>
          <td>{apartment.price}</td>
          <td>{apartment.disctrict}</td>
          <td>{apartment.address}</td>
           <td>{apartment.status} <span onClick={() => updateStatus(apartment.id)}><i class="fas fa-pen"></i></span></td>
          <td>
            <div className="row">
               <div className="col-6"> 
                    <MDBBtn tag='a' onClick={() => remove(apartment.id)} color='none' className='m-1 text-danger'>
                            <i className="fas fa-trash"></i>
                    </MDBBtn>
               </div>
                <div className="col-6">
                      <Link onClick={() => update(apartment.id,apartment.owner,apartment.title, apartment.tel, apartment.discription, apartment.price, apartment.disctrict, apartment.address, apartment.image)} >
